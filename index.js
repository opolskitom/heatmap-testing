window.onload = function() {
    ...
    var mbUrl = 'https://maps.wikimedia.org/osm-intl/{z}/{x}/{y}.png';
    var mbAttr = 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="http://cloudmade.com">CloudMade</a>';

    var baseLayer = L.tileLayer.colorFilter(
      mbUrl,{
        attribution: mbAttr,
        maxZoom: 12,
        minZoom: 3,
    ...
    var heatmapLayerSmall = new HeatmapOverlay(cfgSmall);
    ...
    heatmapLayerSmall.setData(testData);

    // zooming properties for managing heatmap radius at higher zoom levels
    var currentZoom = map.getZoom();
    var smallZoom = false;
    var changeRadiusAt = 7;
    map.addEventListener("zoom", changeZoom)
    ...
    function changeZoom() {
        ...
        if (!smallZoom && currentZoom > changeRadiusAt) {
            smallZoom = true;
            heatmapLayer.remove();
            heatmapLayerSmall.addTo(map);
        } else if (smallZoom && currentZoom <= changeRadiusAt) {
            smallZoom = false;
            heatmapLayerSmall.remove();
